...
import { userLogin } from '../../utils/slice/UserSlice';
...
const MasterLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userData = response.data.find((key)=> key.email === email)

      if (response.status === 200) {

        let udata= userData;
         console.log(udata);
        dispatch(userLogin(udata))
        navigate('/dashboard');
      } else {
      ...
      {error && <p>{error}</p>}
        ...
        <div className="flex items-center text-lg mb-6 md:mb-8">
          ...
          <input type="password" id="password" value={password}
            ...
            className="bg-gray-200 rounded pl-12 py-2 md:py-4  focus:outline-none w-full" placeholder="Password"  required />
        </div>
