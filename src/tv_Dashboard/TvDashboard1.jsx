  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${DjangoConfig.apiUrl}/rtqm/sewing_tv_dashboard/?line_id=${line_id}&process_id=${process_id}&section_id=${section_id}`, {
          headers: {
      ...
      <header>
        <nav className="bg-[#070F2B] border-2 border-gray-600 w-full h-16 flex items-center">
          ...
          <div className="w-3/5 h-full flex items-center justify-center border-r border-black">
            <span className="text-goldenrod text-3xl font-bold"> {planData.process_name} Dashboard (UNDER TESTING)</span>
          </div>
        ...
        <div className="bg-black border border-gray-500 w-full h-7vh flex justify-between pt-2 pb-2 items-center mt-1">
          ...
          <div className="w-1/6 h-full flex items-center justify-center">
            <span className="text-white text-2xl font-semibold">RD : {formulasData.run_day}</span>
          </div>|
          <div className="w-1/6 h-full flex items-center justify-center">
          ...
          <div className="w-1/3 h-full flex flex-col">
            <div className="border-2  border-gray-500 w-full pl-3 flex flex-col items-start justify-evenly h-[64vh]">
              <div className='w-full flex justify-between items-center'>
                ...
                <div className='font-bold text-7xl pr-6'>
                  
                  {formulasData.day_target || '00'
          ...
          <div className="w-[45%] h-full flex flex-col ml-3">
            <div className="flex w-full h-[64vh]">
              ...
              <div className="border-2 border-gray-500 w-1/2 h-full ml-3 ">
                ...
                <div className='font-bold text-8xl text-center text-white pt-4 mt-4'>
                {formulasData.eff_percentage?formulasData.eff_percentage:(<div className='animate-pulse text-error text-red-700'>00</div>)}
                </div>
                <div className='flex w-full flex-col justify-center items-center'>
                  <div style={{ width: 150, height: 120 }} className='mt-4 '>
                    <CircularProgressbar
                      ...
                      styles={buildStyles({
                        ...
                        pathColor: formulasData.eff_percentage > formulasData.standard_efficiency ? 'green' : 'red',
                        textColor: formulasData.eff_percentage > formulasData.standard_efficiency ? 'green' : 'red',
                        trailColor: '#fff',
                        backgroundColor: '#3e98c7',
                      })}
                    />
            ...
            <div className="flex w-full h-[12vh] mt-1">
              <div className="border-2 border-gray-500 w-1/2 h-full flex justify-between items-center">
                <div className="text-chocolate text-whitetext-white text-2xl font-semibold pl-2"> Tailor</div>
                <div className='font-bold text-7xl  text-white pr-5'>{formulasData.operator_count?formulasData.operator_count:(<div className='animate-pulse text-error text-red-700'>00</div>)}</div>
              </div>
              <div className="border-2 border-gray-500 w-1/2 h-full ml-3 flex justify-between items-center">
                ...
                <div className='font-bold text-7xl  text-white pr-5'>{formulasData.helper_count?formulasData.helper_count:(<div className='animate-pulse text-error text-red-700'>00</div>)}</div>
              </div>
            </div>
          </div>

          <div className="w-1/5 h-full ml-3 flex flex-col">
            <div className="border-2 border-gray-500 w-full h-[38vh]">
              <span className="text-chocolate text-white text-2xl font-semibold p-4">DHU%</span>
              <div>
                <h2 className={`font-bold text-6xl text-center pt-4 mt-10 ${formulasData.dhu_percentage < 5 ? 'text-green-500' : 'text-red-500'}`}>
                  {formulasData.dhu_percentage || '00'}
                </h2>
              </div>
            </div>
            <div className="border-2 border-gray-500 w-full h-[39vh] mt-1">
              <div>
            ...
            <div className="border-2 border-gray-500 w-full mb-4 justify-between items-center flex h-[15vh] mt-1">
              <div className="text-chocolate text-white  text-2xl font-semibold pl-2">Pressman </div>
              <div className='font-bold text-7xl  text-white pr-5'>{formulasData.pressman_count?formulasData.pressman_count:(<div className='animate-pulse text-error text-red-700'>00</div>)}</div>
            </div>
          </div>
        </div>
      </header>
