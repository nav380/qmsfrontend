...
const TVDashboard3 = () => {
    ...
    const rows = sortedData.slice(0, breakIndex).map((item) => {
        ...
        return (
            <tr key={item.hour_no}>
                ...
                <td>{formatValue(formatToTwoDecimals(item.difference_eff))}</td>
    ...
    return (
        <section id="dashboard" className="bg-black h-screen" ref={dashboardRef}>
            <header>
                <nav className="bg-[#070F2B] border-2 border-slate-500 w-full h-15 flex items-center justify-between">
                    ...
                    <div className="flex-grow flex items-center justify-center border-r border-black w-3/5 h-full">
                        <span className="text-goldenrod font-bold text-white text-3xl blink">
                            Production DashBoard (UNDER TESTING)
                        </span>
                    </div>
            ...
            <button
                className="absolute top-[88px] left-1 transform -translate-y-1/2 bg-transparent text-white px-4 py-2 text-xl  focus:outline-none hover:bg-gray-700"
