...
import { addOperation } from "../../../utils/slices/DefectMasterSlice";
import DjangoConfig from "../../../config/Config";
import { userSelectId } from "../../../utils/slices/userSlice";

const DefectMasterSelect = () => {
  ...
  // const userId = useSelector(userSelectId);
  ...
  useEffect(() => {
    loadStoredData()
    fetchDefectOperations(operationId); // Pass operationId to fetchDefectOperations
  }, [operationId]); 
...
let matchedOptions = [];
  ...
  return (
    ...
    //   <div className="flex flex-wrap p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
    //     {
    //       options.map((option, index) => (
    //         <div
    //           key={index}
    ...
    //           onClick={() => toggleOption(option)}
    //         >
    //           <div className={`border border-gray-300 px-3 py-2 rounded-md cursor-pointer ${
    //             (selectedOptions.some(opt => opt.id === option.id) || matchedOptions.some(opt => opt.id === option.id)) ? "bg-blue-200" : "bg-white"
    //           }`}>
    //             {option.name}
    //           </div>
    //         </div>
    ...
    //     <button className="btn btn-success" onClick={handleSaveAddMore}>Add More </button>
    //     <button className="btn btn-success" onClick={handleSaveAndNext}>Next</button>

    //     <button className="btn btn-warning" onClick={handleClearLocalStorage}>Clear Local Storage</button>
    //     {/* <button onClick={handelSaveAllData}>Next</button> */}
    //     </div>
    // </div>
  );
};

export default DefectMasterSelect;
...
{/* <div className="flex flex-wrap p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
          {options.map((option, index) => (
            <div
              key={index}
              className={`border border-gray-300 px-3 py-2 rounded-md cursor-pointer ${
                selectedOptions.some(opt => opt.id === option.id) ? "bg-blue-200" : "bg-white"
              }`}
              onClick={() => toggleOption(option)}
            >
              {option.name} 
