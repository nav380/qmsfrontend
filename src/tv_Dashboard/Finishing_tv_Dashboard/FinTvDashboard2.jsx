...
const FinTvDashboard2 = () => {
  ...
  const toggleFullScreen = () => {
    if (!document.fullscreenElement) {
      dashboardRef.current.requestFullscreen?.() || dashboardRef.current.mozRequestFullScreen?.() || 
      dashboardRef.current.webkitRequestFullscreen?.() || dashboardRef.current.msRequestFullscreen?.();
    } else {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`${DjangoConfig.apiUrl}/rtqm/final_tv_dashboard2/?line_id=${line_id}&process_id=${process_id}&section_id=${section_id}`, {
          headers: {
            'Content-Type': 'application/json',
          },
        });

        console.log("Silhouettes data length: ", response.data);
  ...
  const updateData = (data) => {
    setDefectList([])
    setObList([])
    const { silhouettes_data, today_defect_list, plan_data, defect_list, ob_list } = data;
  ...
  return (
    <section id="dashboard" className="bg-black h-screen" ref={dashboardRef}>
      <header>
        <nav className="bg-[#070F2B] border-2 border-slate-500 w-full h-16 flex items-center justify-between">
          ...
          <div className="flex-grow flex items-center justify-center border-r border-black w-3/5 h-full">
            <span className="text-goldenrod font-bold text-white text-3xl">Finishing Dashboard</span>
          </div>
        ...
        <div className="flex h-full">
          ...
          <div className="w-1/3 h-full flex flex-col mt-2 items-center justify-between">
            <div className="border-2 border-slate-500 min-h-40 w-full h-1/2 items-center justify-center">
              <span className="text-chocolate text-4xl text-slate-200 font-semibold mb-2">Top 3 Defects</span>
              <hr className='mt-2' />
            ...
            <div className="border-2 border-slate-500 w-full max-h-1/2 min-h-40 mt-3 items-center justify-center">
              <span className="text-chocolate text-4xl text-slate-200 font-semibold mb-2">Critical to Quality</span>
              <hr className='mt-2' />
              <ul className="text-white text-2xl">
...
export default FinTvDashboard2;
